...
The `@mastra/slack` package includes `SlackSignals`, a polling provider that watches Slack conversations as your Slack user account and wakes agent threads when new messages arrive. It needs no webhooks or public endpoints, and it persists subscriptions and last-seen cursors on thread metadata so restarts don't re-deliver old messages.
...
`SlackSignals` authenticates as a Slack user against a pre-existing Slack app using a PKCE OAuth connect flow, or accepts a static user token for headless environments. It exposes `slack_subscribe_thread`, `slack_unsubscribe_thread`, and `slack_list_subscriptions` tools so the agent can manage its own subscriptions.
