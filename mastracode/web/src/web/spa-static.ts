 ...
 *   2. `ui/` next to the bundled server module — `web:build` copies the vite
 *      output into `.mastra/output/ui` so the build output is self-contained.
 *   3. `dist/web/ui` under cwd — the vite outDir when running `web:start`
 *      from the package root.
 * Returns `undefined` when no build is found (e.g. plain `mastra dev`), in
 * which case the middleware is simply not mounted.
 */
export function resolveUiDistDir(): string | undefined {
  const candidates = [
    ...
    resolve(process.cwd(), 'dist/web/ui'),
  ];
